...
export const fetchCrimeFromServer = action$ =>
    action$.ofType('FETCHING_DATA')
        .switchMap(() => {
            return Observable.fromPromise(firebase.database().ref().child('crime').once('value'))
                .map(u => {
                    console.log(u);
                    return {
                        type: 'FETCH_CRIME',
...
export const fetchMissingFromServer = action$ =>
    action$.ofType('FETCHING_DATA')
        .switchMap(() => {
                return Observable.fromPromise(firebase.database().ref().child('missing').once('value'))
                    .map(u => {
                        console.log(u);
                        return {
                            type: 'FETCH_MISSING',
...
export const fetchComplaintFromServer = action$ =>
    action$.ofType('FETCHING_DATA')
        .switchMap(() => {
                return Observable.fromPromise(firebase.database().ref().child('complain').once('value'))
                    .map(u => {
                        console.log(u);
                        return {
                            type: 'FETCH_COMPLAIN',
